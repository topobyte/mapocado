...
public class RunMapViewerSimple
{
	static final Logger logger = LoggerFactory
			.getLogger(RunMapViewerSimple.class);
	...
	private static final String OPTION_SCALEFACTOR = "scalefactor";
	private static final String OPTION_TILESIZE = "tilesize";
	...
	public static ExeOptionsFactory OPTIONS_FACTORY = new ExeOptionsFactory() {
		...
		public ExeOptions createOptions()
		{
			...
			OptionHelper.addL(options, OPTION_SCALEFACTOR, true, false, "float", "factor to multiply all sizes with");
			OptionHelper.addL(options, OPTION_TILESIZE, true, false, "int", "size of map tiles in pixels");
			// @formatter:on
	...
	public static void main(String name, CommonsCliArguments arguments)
	{
		...
		float scaleFactor = 1;
		if (line.hasOption(OPTION_SCALEFACTOR)) {
			String value = line.getOptionValue(OPTION_SCALEFACTOR);
			float sf = Float.parseFloat(value);
			if (sf != 0) {
				scaleFactor = sf;
			}
		}
		...
		if (line.hasOption(OPTION_TILESIZE)) {
			String value = line.getOptionValue(OPTION_TILESIZE);
			NumberFormat numberFormat = NumberFormat.getInstance(Locale.US);
			try {
				tileSize = numberFormat.parse(value).intValue();
			} catch (java.text.ParseException e) {
				System.out.println("unable to parse number: " + value);
				System.exit(1);
			}
			System.out.println("tilesize: " + tileSize);
		}

		MapViewerSimple mapViewer = new MapViewerSimple(tileSize);
		...
		mapViewer.getMapocadoViewer().setScaleFactor(scaleFactor);
	}
}
